...
def train(cfg: DictConfig):
    ...
    for n in range(2,6):
        env_args['dataset_path'] = to_absolute_path('maps/datasets/map1/' + str(n) + '.pth')
        env_args["number_agents"] = n
        ...
        net = MyNet(n_agents=n, centralized=False, share_params=True, device=cfg.model.device)
        module = TensorDictModule(
            net, in_keys=[("agents", "observation"), ("agents", "t_to_go"), ("agents", "last_action"),
                          ("agents", "out_of_bounds")], out_keys=[("agents", "action_value")])
        ...
        truncated = False
        for t in range(50):
            env_args['dataset_path'] = to_absolute_path('maps/datasets/map4/' + str(n) + '.pth')
            env_args["number_agents"] = n
            env = Environment(EnvironmentParams(env_args))
            env = TorchRLEnvironmentWrapper(env)
            env.env.env_params.state_ptr = 0
            ...
            while True:
                if done or truncated:
                    ...
                    env.env.position_locked = [False for _ in range(env.env.params.number_agents)]
                    env.env.heuristic_position = [None for _ in range(env.env.params.number_agents)]
                    tiles = set(env.env.state.global_map.getTiles()).difference(env.env.state.global_map.obstacle_list)
                    env.env.save_position = random.choice(list(tiles))
                ...
                if any(info):
                    ...
                    for i, a in enumerate(ac):
                        ...
                        if  action["agents"]["action"][i] == 4 and info[i] == False:
                            pass #print(action, info, env.env.state.remaining)
                ...
                if done or truncated:
                    if not truncated:
                        print(" time_save: ", env.env.rewards.get_time_save(), env.env.saved)
                        timesave[n].append(env.env.rewards.get_time_save())
                        saved[n].append(env.env.saved)
                    break
